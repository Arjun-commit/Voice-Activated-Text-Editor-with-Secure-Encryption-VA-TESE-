...
import Notepad_file_controller
import Notepad_db_controller
import traceback
...
class Notepad:
    ...
    def set_tool_bar(self):
        ...
        ##now time
        self.time_ = tk.Label(self.tool_bar, font = ('calibri', 15, 'bold'), relief = "raised", bg = "lightblue")
        def Time():

            string = time.strftime('%H:%M:%S %p')
            self.time_.config(text = string)
            self.time_.after(1000, Time)

        self.time_.grid(row = 0, column=10, padx = 10, )
    ...
    def set_status_bar(self):
        self.status_bar = ttk.Label(self.root, text='Status Bar', anchor = "w")
    ...
    def set_controllers(self):
        self.file_controller = Notepad_file_controller.File_controller()
        self.db_controller = Notepad_db_controller.Db_controller()
    ...
    def exit_func(self, event=None):
        try:
            ...
            messagebox.showinfo("Thankyou", "Have a good day")
    ...
    def changed(self, event=None):
        if self.text_editor.edit_modified():
            ...
            self.status_bar.config(text=f'Characters:{characters} Words:{words}', anchor = "w")
    ...
    def say_something(self):
        try:
            ...
            if self.take_audio == "":
                messagebox.showinfo("say again", "Speech is recognized")
            elif self.take_audio.lower() == "hey mojo open file":
                self.open_file()
            elif self.take_audio.lower() == "hey mojo new file":
                self.new_file()
                ...
                self.change_italic()
            elif self.take_audio.lower() == "hey mojo text underline":
            ...
            messagebox.showerror("Timeout", "You have taken to much time.")

        except UnknownValueError as unknw:
            messagebox.showerror("Translation problem", "We don't recognize what you have said.")
    ...
    def secure_file(self, event=None):
        if self.db_status:
            self.file_dialoge = tk.Toplevel()
            self.file_dialoge.geometry('450x400+500+200')
            self.file_dialoge.title('Secure file')
            self.file_dialoge.resizable(0,0)

            #frame
            self.file_frame = ttk.LabelFrame(self.file_dialoge, text = "Add Secure file")
            ...
            #entry grid
            self.file_owner_input.grid(row=0, column = 0 )
            self.file_pwd_input.grid(row=1, column = 0)

            #button
            self.file_add_button.grid(row=4, column = 0, padx=4, pady = 4)
            self.file_open_button.grid(row=4, column = 1, padx=4, pady = 4)
            self.file_remove_button.grid(row=4, column = 2, padx=4, pady = 4)

            #scrollbar
            scrollbar = ttk.Scrollbar(self.file_dialoge, orient = "vertical")
            scrollbar.pack(side='right', fill= 'both')
            self.fileList = tk.Listbox(self.file_dialoge)
            ...
            if len(self.sel_file_index_tuple)==0:
                ...
                if self.db_controller.is_file_secure(basename):
                    ...
                    if pwd==self.db_controller.get_file_pwd(basename):
                        self.text_editor.delete(1.0,tk.END)
                        self.text_editor.insert(1.0,tk.END)
    ...
    def remove_secure_file(self):
        try:
            ...
            messagebox.showerror("Secure file Error", "Error while removing the file")
            print(traceback.format_exc())
    ...
    def add_file(self):
        try:
            if self.file_owner_input.get()!="" and self.file_pwd_input.get()!="":
                ...
                self.file_path = filedialog.askopenfilename(title = "Select file", filetype = [("Text Document","*.*")])
                ...
                if result.find("already")!=-1:
                    ...
                    messagebox.showerror("Success!",result)
            else:
                messagebox.showerror("Error!", "Please input file owner and password before adding file")

        except:
            messagebox.showerror("Error!", "Error while adding file")
            print(traceback.format_exc())
    ...
    def get_file_pwd(self):
        access_pwd = simpledialog.askstring("Password", "Enter the file password:", show = "*")
        return access_pwd
